...
(function (SAP) {
  ...
  function kademeli(secici, host, y) {
    ...
    gsap.set(els, { autoAlpha: 0, y: y || 56 });
    ST.batch(els, {
      start: 'top 90%',
      once: true,
      onEnter: function (grup) {
        gsap.to(grup, { autoAlpha: 1, y: 0, duration: 1, ease: EASE_IN, stagger: 0.09, overwrite: true });
      },
    });
  }
    ...
    ctx = gsap.context(function () {
      ...
      if (route.name === 'home') {
        ...
        gsap.from(host.querySelectorAll('.hero-copy > :not(.hero-t)'),
                  { autoAlpha: 0, y: 28, duration: 1, ease: EASE_IN, stagger: 0.1, delay: 0.35 });
        gsap.from(host.querySelector('.hero-tool'),
                  { autoAlpha: 0, y: 64, duration: 1.2, ease: EASE_IN, delay: 0.25 });
        /* Derinlik: kaydırınca komut paneli metinden biraz yavaş kalkar. */
        gsap.to(host.querySelector('.hero-tool'), {
          yPercent: -10, ease: 'none',
          scrollTrigger: { trigger: host.querySelector('.hero'), start: 'top top', end: 'bottom top', scrub: true },
        });
      ...
      if (route.name === 'konu') {
        ...
        gsap.from(host.querySelectorAll('.thead > :not(h1)'),
                  { autoAlpha: 0, y: 24, duration: 0.9, ease: EASE_IN, stagger: 0.08, delay: 0.25 });
