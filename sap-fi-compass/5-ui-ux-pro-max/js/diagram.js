    ...
    var ents = d.varliklar.map(function (v) {
      var fields = (v.alanlar || []).map(function (f) {
        ...
        return '<li>' + badge + '<span>' + esc(f.ad) + '</span>' +
               (f.not ? '<span style="font-family:var(--font-sans);color:var(--ink-3);font-size:12px">' +
                        mk(f.not) + '</span>' : '') + '</li>';
      }).join('');
    ...
    var rels = (d.iliskiler || []).map(function (r) {
      return '<div class="er-rel">' +
        ...
        /* Ok, karakterle ÇİZİLMEZ. Eskiden `──▶` yazıyordu — kutu çizim
           ...
           ve baskıda dağılan bir çözüm. Artık Phosphor `arrow-right` (16px,
  ...
  function fis(d) {
    ...
    var rows = (d.satirlar || []).map(function (r) {
      ...
      return '<tr>' +
        ...
        '<td>' + mk(r.ad || '') + (r.not ? '<br><span style="font-size:12px;color:var(--ink-3)">' + mk(r.not) + '</span>' : '') + '</td>' +
