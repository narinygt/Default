...
(function (SAP) {
  ...
  function notesHTML(topicId) {
    ...
    return '<div class="notes">' +
      '<label class="notes-l" for="note-' + esc(topicId) + '">' + esc(T('notes.mine')) + '</label>' +
      '<textarea id="note-' + esc(topicId) + '" data-note="' + esc(topicId) + '" rows="6" ' +
        'aria-describedby="note-help-' + esc(topicId) + '" ' +
        'placeholder="' + esc(T('notes.ph')) + '">' + esc(v) + '</textarea>' +
      '<div class="notes-f" id="note-help-' + esc(topicId) + '"><span data-note-status aria-live="polite">' +
