...
(function (SAP) {
  ...
  /* ======================================================== TEMA ====
     Site TEK TEMALIDIR: her zaman aydınlık (beyaz). Karanlık tema ve
  ...
  function openPalette() {
    ...
    pal.innerHTML = '<div class="palette" role="dialog" aria-modal="true" aria-label="' + esc(SAP.i18n.t('nav.search')) + '">' +
  ...
  function setMenu(open) {
    ...
    /* Menü açıkken arkadaki içerik odak dışı (inert); odak menüye girer,
       kapanınca menü düğmesine döner (escape-routes, focus-management). */
    var v = document.getElementById('view');
    if (v) v.inert = !!open;
    if (open) {
      var ilk = sb.querySelector('.side-nav a, .side-nav button');   /* marka mobilde gizli */
      if (ilk) setTimeout(function () { ilk.focus({ preventScroll: true }); }, 60);
    } else if (mb && sb.contains(document.activeElement)) {
  ...
  /* Atlama bağlantısı: href="#view" yönlendiriciye rota gibi görünür; bu
     yüzden varsayılanı engellenir ve odak doğrudan içeriğe taşınır. */
  ...
  function chromeHTML() {
    var T = SAP.i18n.t;
    var mb = document.getElementById('menu-btn');
    /* İki çizgili hamburger — mobilde açılınca çizgiler dönüp X olur
       (app.css .burger). Çizgiler span, ikon değil: morph için gerekli. */
    if (mb) { mb.innerHTML = '<span class="burger" aria-hidden="true"><i></i><i></i></span>'; mb.title = T('nav.menu'); mb.setAttribute('aria-label', T('nav.menu')); }

    var sk = document.getElementById('skip-link');
    if (sk) sk.textContent = T('nav.skip');
    var cr = document.getElementById('crumb');
    if (cr) cr.setAttribute('aria-label', T('nav.crumb'));
