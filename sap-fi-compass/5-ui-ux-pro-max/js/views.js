...
(function (SAP) {
  ...
  SAP.view('home', function () {
        ...
        return '<div class="idx-wrap">' +
          '<button class="idx-row' + (p >= 100 ? ' read' : '') + '" type="button" ' +
            ...
            '<span class="idx-m">' +
              ...
              /* Durum yalnızca renkle değil METİNLE de verilir (color-not-only):
                 ekran okuyucu için gizli etiket, fare için title. */
              '<span class="idx-dot' + durum + '" title="' + esc(T(p >= 100 ? 'idx.read' : p > 0 ? 'idx.part' : 'idx.unread')) + '"></span>' +
              '<span class="sr-only">, ' + esc(T(p >= 100 ? 'idx.read' : p > 0 ? 'idx.part' : 'idx.unread')) + '</span>' +
            '</span>' +
          '</button>' +
    ...
    return '<div class="wrap-full">' +
      ...
      /* Asimetrik bölünmüş karşılama: solda mesaj + tek ana eylem, sağda
         GERÇEK bileşen (canlı komut alanı) — sahte ekran görüntüsü yok.
         Genel ilerleme burada tekrarlanmaz; kenar çubuğunun altında durur. */
      '<section class="hero">' +
        '<div class="hero-copy">' +
  ...
  SAP.action('goto-section', function (btn) {
    ...
    if (!el) return;
    /* Lenis açıksa kaydırmayı o yapar (js/motion.js); değilse tarayıcı. */
    if (SAP.motion && SAP.motion.scrollTo(el)) return;
  ...
  SAP.view('tcode', function (route) {
    ...
    return '<div class="wrap">' +
      ...
      (konular.length
        ? '<div class="panel"><h2>' + esc(T('ref.usedIn')) + '</h2>' + relatedGrid(konular.map(function (t) { return t.id; })) + '</div>'
        : (x.konu && SAP.topic(x.konu)
            ? '<div class="panel"><h2>' + esc(T('ref.usedIn')) + '</h2>' + relatedGrid([x.konu]) + '</div>' : '')) +

      (ayniKonu.length
        ? '<div class="panel"><h2>' + esc(T('ref.siblings')) + '</h2>' + U.chips(ayniKonu.slice(0, 24)) + '</div>'
        : '') +
    '</div>';
  });
  ...
  SAP.view('tablo', function (route) {
    ...
    return '<div class="wrap">' +
      ...
      (x.alanlar && x.alanlar.length
        ? '<div class="panel"><h2>' + esc(T('ref.keyFields')) + '</h2>' +
      ...
      (konular.length
        ? '<div class="panel"><h2>' + esc(T('ref.tableInTopics')) + '</h2>' + relatedGrid(konular.map(function (t) { return t.id; })) + '</div>'
        : (x.konu && SAP.topic(x.konu)
            ? '<div class="panel"><h2>' + esc(T('ref.usedIn')) + '</h2>' + relatedGrid([x.konu]) + '</div>' : '')) +
    '</div>';
  });
  ...
  SAP.view('terim', function (route) {
    ...
    return '<div class="wrap">' +
      ...
      '<div class="panel"><h2>' + esc(T('ref.definition')) + '</h2>' +
        '<div class="prose">' + mkp(A(x, 'aciklama')) + '</div>' +
        (x.detay ? '<div class="prose" style="margin-top:12px">' + mkp(A(x, 'detay')) + '</div>' : '') +
      '</div>' +

      (iliskili.length
        ? '<div class="panel"><h2>' + esc(T('ref.related')) + '</h2>' + U.chips(iliskili) + '</div>'
        : '') +

      (x.konu && SAP.topic(x.konu)
        ? '<div class="panel"><h2>' + esc(T('ref.termInTopic')) + '</h2>' + relatedGrid([x.konu]) + '</div>'
        : '') +
    '</div>';
  });
      ...
      (ids.length
        ? ids.map(function (id) {
            ...
            return '<div class="panel">' +
              /* Bağlantı başlığın DIŞINDA: başlığın erişilebilir adı yalnızca konu adı olsun. */
              '<div class="panel-h"><h2>' + esc(bas(t)) + '</h2>' +
                '<a class="btn sm" data-go="#/konu/' + esc(id) + '" ' +
                'href="#/konu/' + esc(id) + '">' + esc(T('home.start')) + '</a></div>' +
  ...
  /* Giriş hareketi artık js/motion.js'te (GSAP + ScrollTrigger). */
